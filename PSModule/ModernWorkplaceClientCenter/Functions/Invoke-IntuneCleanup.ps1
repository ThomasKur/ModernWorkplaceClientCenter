...
function Invoke-IntuneCleanup {
    ...
    Begin {
        ...
        try{
            ...
            Throw "Not authenticated.  Please use the `"Connect-MSGraph`" command to authenticate first or if required install the Microsoft.Graph.Intune PSModule from the PSGallery."
        }
    }
    Process {
        ...
        foreach($duplicatedDevice in $duplicatedDevices){
            ...
            foreach($oldDevice in ($duplicatedDevice.Group | Sort-Object -Property lastSyncDateTime -Descending | Select-Object -Skip 1)){
                ...
                if($WhatIfPreference){
                    ...
                    if ($PSCmdlet.ShouldProcess("Intune Device $($oldDevice.id)",'Remove')) {
